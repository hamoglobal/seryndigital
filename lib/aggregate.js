...
export function buildBuckets(days, mode, sourcesByDay) {
  ...
  // matching the source-list modals, which already dedupe via dedupeSources.
  // Per-category (not "dedupe the total, then filter by each source's latest
  // sentiment") so every count here is computed exactly the way the matching
  // click-through modal computes modalTotalCount (see sourcesForBucket +
  // dedupeSources usage in Dashboard.jsx) — the KPI card value must always
  // equal the number of rows the user sees when they click into it.
  if (sourcesByDay) {
    buckets = buckets.map(b => ({
      ...b,
      total: dedupeSources(sourcesForBucket(sourcesByDay, b, 'total')).length,
      positive: dedupeSources(sourcesForBucket(sourcesByDay, b, 'positive')).length,
      neutral: dedupeSources(sourcesForBucket(sourcesByDay, b, 'neutral')).length,
      negative: dedupeSources(sourcesForBucket(sourcesByDay, b, 'negative')).length,
      newSources: dedupeSources(sourcesForBucket(sourcesByDay, b, 'new')).length,
    }));
  }
  return buckets;
}
...
export function sourcesForBucket(sourcesByDay, bucket, category) {
  ...
  if (category === 'positive') return countable.filter(s => s.sentiment === 'positive');
  if (category === 'neutral') return countable.filter(s => s.sentiment === 'neutral');
  if (category === 'negative') return countable.filter(s => s.sentiment === 'negative');
  if (category === 'new') return countable.filter(s => s.isNew);
  return countable;
}
