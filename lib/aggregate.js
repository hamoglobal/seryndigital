...
export function buildBuckets(days, mode, sourcesByDay) {
  ...
  // already uses everywhere else) makes Tích cực/Trung tính/Tiêu cực/Chưa
  // phân loại a true, mutually-exclusive partition of the total.
  //
  // `unclassified` covers rows whose sentiment the parser couldn't determine
  // (normalizeSentiment()'s 'unknown' fallback in parser.mjs — e.g. a report
  // date whose "Đánh giá" column wasn't detected due to header drift). Those
  // sources are real and belong in Tổng nguồn, but have no sentiment to put
  // in Tích cực/Trung tính/Tiêu cực — without surfacing this count
  // separately, Tổng nguồn would exceed the visible breakdown with no
  // explanation (e.g. positive% + neutral% + negative% not summing to 100%
  // for a period like "Năm" that spans one of those report dates).
  if (sourcesByDay) {
    buckets = buckets.map(b => {
      const deduped = dedupedSourcesForBucket(sourcesByDay, b);
      const positive = sourcesForBucket(deduped, 'positive').length;
      const neutral = sourcesForBucket(deduped, 'neutral').length;
      const negative = sourcesForBucket(deduped, 'negative').length;
      return {
        ...b,
        total: deduped.length,
        positive, neutral, negative,
        unclassified: deduped.length - positive - neutral - negative,
...
export function sourcesForBucket(deduped, category) {
  ...
  if (category === 'unclassified') return deduped.filter(s => !['positive', 'neutral', 'negative'].includes(s.sentiment));
