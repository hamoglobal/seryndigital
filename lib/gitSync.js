...
// read/write on this repo only). Two sources, in priority order:
//   1. GITHUB_PAT env var — this is what the deployed Render container
//      uses. .ssh/github_pat.txt is git-ignored (deliberately — never want
//      a push token baked into the Docker image via git), so a container
//      built fresh from the repo has no such file; without this env var,
//      role changes made on the live admin page would only ever live on
//      that container's ephemeral disk and vanish on the next deploy.
//   2. .ssh/github_pat.txt file — used by the local dev machine's
//      scheduled ingestion pipeline (scripts/watch-ingest.mjs), which runs
//      outside any container and already has this file on disk.
// IMPORTANT: never log the raw remote URL or raw err.message — both can
// contain the token in plain text. Always redact via redactToken() first.
...
function hasToken() {
  return !!process.env.GITHUB_PAT || fs.existsSync(tokenPath);
}
...
export function syncToGitHub({ message } = {}) {
  if (!hasToken()) {
    console.log('[git-sync] no GitHub PAT found (GITHUB_PAT env var or .ssh/github_pat.txt) — skipping (GitHub push not set up).');
