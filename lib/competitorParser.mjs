...
// Returns { brands, hasBadNewsCol, hasNewArticlesCol } — the two "has*Col"
// flags let the caller know whether "Số tin xấu"/"Số bài viết mới" were
// genuinely absent from this file's overview/summary sheet (as opposed to
// present but legitimately 0), so it can backfill from the parsed item list
// instead of silently showing 0 for every brand (see parseCompetitorReportFile).
function parseOverviewSheet(matrix) {
  if (!matrix) return { brands: [], hasBadNewsCol: false, hasNewArticlesCol: false };
  const headerIdx = findHeaderRowIndex(matrix);
  if (headerIdx === -1) return { brands: [], hasBadNewsCol: false, hasNewArticlesCol: false };
  ...
  return { brands: out, hasBadNewsCol: col.badNews !== -1, hasNewArticlesCol: col.newArticles !== -1 };
}

/**
 * New report format (from ~2026-07-13 onward, produced by the "baocao-tham-my-hang-ngay"
 * 9:10 AM task that replaced the old 21:00 "bao-cao-tham-my-hang-ngay" task): a single
 * combined detail sheet ("Báo cáo tổng hợp") with one row per item and a per-row "Loại"
 * column ("Tin xấu" / "Bài viết mới") instead of two separate sheets. No dedicated
 * "title" column — the full text lives in "Nội dung".
 */
function parseCombinedDetailSheet(matrix) {
  if (!matrix) return [];
  const headerIdx = findHeaderRowIndex(matrix);
  ...
  const col = {
    brand: colIndexFor(header, ['thuong hieu']),
    loai: colIndexFor(header, ['loai']),
    content: colIndexFor(header, ['noi dung', 'tom tat', 'mo ta chi tiet']),
  ...
  for (let r = headerIdx + 1; r < matrix.length; r++) {
    ...
    const rawBrand = cellStr(row[col.brand]);
    if (!rawBrand || !isRealBrandRow(rawBrand)) continue;
    const loaiNorm = norm(col.loai !== -1 ? row[col.loai] : '');
    const type = (loaiNorm.includes('tin xau') || loaiNorm.includes('vi pham')) ? 'bad_news' : 'new_article';
    const url = col.url !== -1 ? cellStr(row[col.url]) : '';
    out.push({
      brand: canonicalizeBrandName(rawBrand),
      type,
      title: '',
      summary: col.content !== -1 ? cellStr(row[col.content]) : '',
...
export function parseCompetitorReportFile(filePath) {
  ...
  // Legacy format: separate "Tổng quan" overview + "Tin xau - Vi pham" / "Bai viet moi"
  // detail sheets (produced by the old 21:00 task, retired ~2026-07-13).
  const overviewMatrix = sheetToMatrix(wb, ['tong quan']);
  let { brands, hasBadNewsCol, hasNewArticlesCol } = parseOverviewSheet(overviewMatrix);
  let items = [];
  ...
  if (brands.length > 0) {
    ...
    items = [
      ...parseDetailSheet(badNewsMatrix, 'bad_news'),
      ...parseDetailSheet(newArticlesMatrix, 'new_article'),
    ];
  } else {
    // New format: "Tóm tắt theo TH" overview + single combined "Báo cáo tổng hợp"
    // detail sheet with a per-row "Loại" column (produced by the 9:10 AM task).
    const summaryMatrix = sheetToMatrix(wb, ['tom tat theo th', 'tom tat']);
    ({ brands, hasBadNewsCol, hasNewArticlesCol } = parseOverviewSheet(summaryMatrix));
    const combinedMatrix = sheetToMatrix(wb, ['bao cao tong hop']);
    items = parseCombinedDetailSheet(combinedMatrix);
  }

  if (brands.length === 0) {
    throw new Error(`No brand rows found in overview/summary sheet (checked "Tổng quan" and "Tóm tắt theo TH"): ${filePath}`);
  }

  // Seen 2026-07-30: the overview/summary sheet dropped its "Số tin xấu" /
  // "Số bài viết mới" columns entirely (not just renamed — the columns were
  // just gone), so every brand silently got badNews=0/newArticles=0 even
  // though the detail sheet's per-item rows were complete. Per-brand item
  // counts are backfilled from the (separately parsed, authoritative) item
  // list ONLY when the corresponding overview column was missing outright —
  // days where the column is present keep using the overview sheet's own
  // count as-is, since it has occasionally differed from a naive per-item
  // tally on other days (e.g. 2026-07-29) for reasons upstream, not a bug here.
  if (!hasBadNewsCol || !hasNewArticlesCol) {
    const itemCounts = new Map(); // brand -> { bad_news, new_article }
    for (const it of items) {
      const c = itemCounts.get(it.brand) || { bad_news: 0, new_article: 0 };
      c[it.type] = (c[it.type] || 0) + 1;
      itemCounts.set(it.brand, c);
    }
    brands = brands.map(b => {
      const c = itemCounts.get(b.brand);
      return {
        ...b,
        badNews: hasBadNewsCol ? b.badNews : (c?.bad_news || 0),
        newArticles: hasNewArticlesCol ? b.newArticles : (c?.new_article || 0),
      };
    });
  }

  return { date, brands, items };
}
