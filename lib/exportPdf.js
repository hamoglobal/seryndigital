...
/**
 * "Khuyến nghị Seryn" advisory report — headline assessment + prioritized
 * daily-action and weekly-strategy lists (see lib/recommendations.js for how
 * `data.daily`/`data.weekly` are built from live dashboard + competitor data).
 * data: {
 *   eyebrow, title, subtitle, generatedLabel, filename,
 *   postureLabel, postureTone: 'success'|'warning'|'danger'|'brand'|'navy',
 *   headline: string,
 *   daily:  [{ title, detail, tag, tone }],
 *   weekly: [{ title, detail, tag, tone }],
 * }
 */
export async function buildRecommendationsPdf(data) {
  ...
  if (data.postureLabel) {
    layout.ensure(30);
    const y = layout.getY();
    const badgeColor = TONE_COLOR[data.postureTone] || COLOR.navy;
    const badgeSoft = TONE_SOFT[data.postureTone] || COLOR.ivory;
    const label = data.postureLabel.toUpperCase();
    doc.setFont(FONT_NAME, 'bold');
    doc.setFontSize(9);
  ...
  if (data.headline) {
    ...
    doc.setFontSize(9.5);
    setText(doc, COLOR.muted);
    const lines = doc.splitTextToSize(data.headline, CONTENT_W);
    doc.text(lines, MARGIN_X, layout.getY());
    layout.advance(lines.length * 13 + 10);
    setText(doc, COLOR.body);
  }

  function recoSection(heading, items) {
    sectionHeading(doc, layout, heading, `${items.length} khuyến nghị`);
    itemList(doc, layout, items.map(it => ({
      heading: it.title,
      lines: [it.tag, it.detail].filter(Boolean),
      dotColor: TONE_COLOR[it.tone] || COLOR.navy,
    })), { numbered: true, emptyLabel: 'Không có khuyến nghị nào cho mục này.' });
    layout.advance(8);
  }

  recoSection('Hành động hằng ngày', data.daily || []);
  recoSection('Chiến lược hằng tuần', data.weekly || []);
