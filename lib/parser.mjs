...
// NOTE: `account` is listed before `platform` on purpose. `platform`'s 'kenh'
// synonym (added for the 2026-07-30 template, which renamed "Nền tảng" ->
// "Kênh") is a substring of account's own 'ten kenh' synonym (used by an
// older template where the account/channel-name column itself was headed
// "Tên kênh"). detectHeader() assigns columns in field-iteration order, so
// account must get first look at any column containing "kenh" — otherwise
// platform's shorter alias would steal that older template's account column.
const SOCIAL_FIELDS = {
  account: ['tai khoan', 'ten kenh', 'ten trang', 'bai dang', 'channel name'],
  platform: ['nen tang', 'platform', 'kenh'],
...
const ALERT_FIELDS = {
  // 'muc phan loai' added for the 2026-07-30 template ("Mức độ" -> "Mức phân loại").
  level: ['muc do', 'level', 'muc phan loai'],
  // 'van de' added for the 2026-07-30 template ("Loại vấn đề"/"Nội dung" -> "Vấn đề").
  type: ['loai rui ro', 'loai van de', 'category', 'noi dung', 'van de'],
  source: ['nguon', 'link'],
  alertDate: ['ngay phat hien', 'ngay', 'date'],
  summary: ['mo ta', 'noi dung tom tat', 'tom tat', 'ly do', 'finding', 'reason'],
  // 'hanh dong' added for the 2026-07-30 template ("Đề xuất xử lý" -> "Hành động tiếp theo").
  // Processed last, so it only ever claims a column nothing else already matched.
  action: ['de xuat xu ly', 'hanh dong de xuat', 'de xuat', 'khuyen nghi', 'recommendation', 'hanh dong'],
};
...
function harvestRowPairKpis(matrix) {
  ...
  for (let r = 0; r < matrix.length - 1; r++) {
    ...
    // First, find every column in THIS row that exactly matches a KPI label
    // (exact match, not "includes" — see note below on why).
    const rowMatches = [];
    for (let c = 0; c < row.length; c++) {
      const n = norm(row[c]);
      if (!n) continue;
      ...
      for (const [field, syns] of Object.entries(ROW_PAIR_KPI_LABELS)) {
        if (syns.some(s => n === s)) { rowMatches.push({ field, c }); break; }
      }
    }

    // A genuine KPI header row (the "TONG NGUON | TICH CUC | TRUNG TINH |
    // CANH BAO DO" layout) always spreads >=2 distinct KPI labels across the
    // row. A single isolated match — e.g. a "⚠️ CẢNH BÁO ĐỎ" section title
    // sitting alone in column A, with a free-text explanatory paragraph in
    // the same column of the row below it — is NOT a KPI header. Treating it
    // as one misreads that paragraph as a numeric KPI value (digits inside
    // the prose, e.g. a street address, get parsed as the figure), which can
    // fabricate a nonzero "negative" count out of a report that actually
    // found zero negative content. Require a real multi-column header before
    // harvesting anything from the row below.
    if (rowMatches.length < 2) continue;

    for (const { field, c } of rowMatches) {
      const val = nextRow[c];
...
function computeDayFields({ pairs, sources, alerts, statusLine, date }) {
  ...
  const negativeKpi = findKpi(pairs, ['tieu cuc / canh bao do', 'noi dung tieu cuc', 'nguon canh bao do', 'nguon tieu cuc', 'negative']);
