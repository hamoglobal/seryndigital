...
// are auto-promoted to 'admin' the first time they sign in.
...
export async function upsertUserOnLogin({ email, name, image }) {
  ...
  if (existing) {
    ...
    // Re-apply ADMIN_EMAILS on every login, not just at first creation. This
    // recovers auto-admins if the users.json snapshot was ever lost (e.g. an
    // earlier deploy where the git-sync push was silently failing) and the
    // record got re-created as 'pending'. Never downgrades — an admin who
    // manually demoted this email keeps that decision unless it's back in
    // ADMIN_EMAILS.
    const shouldBeAdmin = adminEmailSet().has(norm) && existing.role !== 'admin';
    ...
    await persist(shouldBeAdmin ? `re-promote ${email} to admin (ADMIN_EMAILS)` : `login ${email}`);
    return existing;
  }

  const role = adminEmailSet().has(norm) ? 'admin' : 'pending';
