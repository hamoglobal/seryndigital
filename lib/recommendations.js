...
export function buildSerynRecommendations({
  ...
  const headline = `Với vai trò cố vấn marketing ngành thẩm mỹ – chăm sóc sức khoẻ hơn 10 năm kinh nghiệm, tôi đánh giá Seryn Clinic đang ở trạng thái "${posture.label}" trong ${rangeLabel}: ${total} nguồn được ghi nhận, ${positivePct}% tích cực, ${selBucket.negative} tín hiệu tiêu cực/cảnh báo cần xử lý. ${competitorClause}`;
